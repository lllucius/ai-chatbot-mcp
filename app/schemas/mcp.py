...
class MCPServerCreateSchema(BaseModel):
    ...
    transport: str = Field(default="http", description="Transport protocol (http, stdio, etc.)")
    timeout: int = Field(default=30, ge=1, le=300, description="Connection timeout in seconds")
    config: Optional[Dict[str, Any]] = Field(default_factory=dict, description="Additional server configuration")
...
class MCPServerUpdateSchema(BaseModel):
    ...
    timeout: Optional[int] = Field(None, ge=1, le=300, description="New timeout in seconds")
...
class MCPToolExecutionResultSchema(BaseModel):
    ...
    content: List[Dict[str, Any]] = Field(default_factory=list, description="Execution result content")
