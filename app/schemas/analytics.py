...
class AnalyticsOverviewResponse(BaseModel):
    ...
    conversations: Dict[str, Any] = Field(
        ..., description="Conversation statistics block"
    )
...
class AnalyticsPerformanceResponse(BaseModel):
    ...
    database_performance: List[Dict[str, Any]] = Field(
        ..., description="Database performance info"
    )
...
class AnalyticsUserAnalyticsResponse(BaseModel):
    ...
    metric: str = Field(
        ...,
        description="Metric used for analytics (messages, documents, conversations)",
    )
    period: str = Field(..., description="Time period for analytics (e.g., 30d)")
    top_users: List[Dict[str, Any]] = Field(
        ..., description="List of top users by metric"
    )
...
class AnalyticsTrendsResponse(BaseModel):
    ...
    daily_trends: List[Dict[str, Any]] = Field(
        ..., description="List of daily trend objects"
    )
...
class AnalyticsExportResponse(BaseModel):
    ...
    system_overview: AnalyticsOverviewResponse = Field(
        ..., description="System overview block"
    )
    usage_statistics: AnalyticsUsageResponse = Field(
        ..., description="Usage statistics block"
    )
    performance_metrics: AnalyticsPerformanceResponse = Field(
        ..., description="Performance metrics block"
    )
