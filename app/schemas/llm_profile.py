...
class LLMProfileCreate(BaseModel):
    ...
    name: str = Field(
        ..., min_length=1, max_length=100, description="Unique profile name"
    )
    title: str = Field(
        ..., min_length=1, max_length=200, description="Human-readable title"
    )
    description: Optional[str] = Field(
        None, max_length=1000, description="Profile description"
    )
    model_name: str = Field(..., description="OpenAI model name")
...
class LLMProfileUpdate(BaseModel):
    ...
    parameters: Optional[Dict[str, Any]] = Field(
        None, description="Model parameters dictionary"
    )
