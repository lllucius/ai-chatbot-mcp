"Service layer for openai_client business logic."
...
class OpenAIClient:
    "OpenAI client for external service communication."
    ...
    async def validate_model_availability(self) -> bool:
        "Validate model availability data."
    ...
    def count_messages_tokens(self, messages: List[Dict[(str, Any)]]) -> int:
        "Count Messages Tokens operation."
        total_tokens = 0
    ...
    async def chat_completion(
        ...
        request_params = {"model": settings.openai_chat_model, "messages": messages}
        if llm_profile:
        ...
        if message.tool_calls:
            ...
            if tool_handling_mode == ToolHandlingMode.RETURN_RESULTS:
                ...
                final_completion = await self._complete_with_tool_results(
                    messages, message.tool_calls, tool_calls_executed, llm_profile
                )
    ...
    async def chat_completion_stream(
        ...
        "Chat Completion Stream operation."
        if (not OPENAI_AVAILABLE) or (not self.client):
            raise ExternalServiceError("OpenAI client not available")
        final_tools = tools or []
        if use_unified_tools and (not tools):
...
[Tool calls completed: {len(tool_calls_executed)} tools executed]""",
    ...
    async def _execute_unified_tool_calls(self, tool_calls) -> List[Dict[(str, Any)]]:
        "Execute Unified Tool Calls operation."
        try:
            tool_executor = await get_unified_tool_executor()
            unified_tool_calls = []
    ...
    def _format_tool_results_as_content(
        ...
        "Format Tool Results As Content operation."
        if not tool_results:
...
"""
    ...
    async def _complete_with_tool_results(
        ...
        "Complete With Tool Results operation."
        ...
        async def _make_final_completion():
            "Make Final Completion operation."
            request_params = {
                "model": settings.openai_chat_model,
    ...
    def _format_tool_result_for_ai(self, result: Dict[(str, Any)]) -> str:
        "Format Tool Result For Ai operation."
    ...
    async def create_embeddings_batch(self, texts: List[str]) -> List[List[float]]:
        ...
        async def _create_batch_embeddings():
            "Create Batch Embeddings operation."
            response = await self.client.embeddings.create(
