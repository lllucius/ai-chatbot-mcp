...
class ConversationService:
    ...
    async def create_conversation(self, request: ConversationCreate, user_id: int) -> Conversation:
        """
        ...
        try:
            ...
            if request.conversation_id:
                ...
                conversation = await self.create_conversation(
                    ConversationCreate(title=title, is_active=True),
                    user_id
                )
    ...
    async def _get_conversation_history(self, conversation_id: int, limit: int = 20) -> List[Message]:
    ...
    async def _get_rag_context(self, request: ChatRequest, user_id: int) -> Optional[List[Dict[str, Any]]]:
