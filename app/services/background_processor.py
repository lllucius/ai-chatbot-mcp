"Service layer for background_processor business logic."
...
class TaskStatus(str, Enum):
    "TaskStatus class for specialized functionality."
...
class ProcessingTask:
    "ProcessingTask class for specialized functionality."
...
class BackgroundProcessor(BaseService):
    "BackgroundProcessor class for specialized functionality."
    ...
    async def queue_document_processing(
        ...
        "Queue Document Processing operation."
    ...
    async def _process_document_task(self, task: ProcessingTask):
        "Process Document Task operation."
        ...
        try:
            ...
            task.progress = 1.0
            logger.info(
                f"Document processing completed for {document.id}: {len(chunks)} chunks, {processing_time:.2f}s"
            )
        except Exception as e:
            (
                await self.db.execute(
    ...
    async def cleanup_completed_tasks(self, max_age_hours: int = 24):
        "Cleanup Completed Tasks operation."
...
async def shutdown_background_processor():
    "Shutdown Background Processor operation."
    global _background_processor
