...
class FastMCPClientService:
    ...
    def _parse_server_configs(self):
        ...
        try:
            ...
            logger.info(f"Configured {len(self.servers)} MCP servers: {list(self.servers.keys())}")
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.info(f"FastMCP initialization completed: {successful_connections}/{len(self.servers)} servers connected")
...
#                logger.warning(f"Failed to discover tools from {server_name}: {e}")
    ...
    def get_tools_for_openai(self) -> List[Dict[str, Any]]:
        ...
        """
        if not self.is_initialized:
            logger.warning("MCP client not initialized - returning empty tools list for OpenAI")
            return []
