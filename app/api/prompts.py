"API endpoints for prompts operations."
...
async def get_prompt_details(
    prompt_name: str, current_user: User = Depends(get_current_user)
):
    "Get prompt details data."
...
async def get_categories(current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        return {"success": True, "data": {"categories": categories, "tags": tags}}
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def set_default_prompt(
    prompt_name: str, current_user: User = Depends(get_current_superuser)
):
...
async def get_prompt_stats(current_user: User = Depends(get_current_user)):
