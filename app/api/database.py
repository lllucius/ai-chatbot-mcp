...
async def get_migration_status(
    ...
    try:
        ...
        available_heads = heads_result.stdout.strip() if heads_result.returncode == 0 else "Unknown"
        ...
        migration_history = history_result.stdout if history_result.returncode == 0 else "Unable to retrieve history"
...
async def create_database_backup(
    output_file: Optional[str] = Query(None, description="Output file path (auto-generated if not provided)"),
    ...
    try:
        ...
        cmd = [
            ...
            "-d", parsed.path.lstrip("/") if parsed.path else "ai_chatbot",
            "-f", output_file,
            "--verbose"
        ]
...
async def restore_database(
    ...
    """
    log_api_call("restore_database", user_id=str(current_user.id), backup_file=backup_file)
            ...
            "data": {
                ...
                "performance_metrics": {
                    ...
                    "transaction_success_rate": round(
                        (stats_row.xact_commit / max(stats_row.xact_commit + stats_row.xact_rollback, 1)) * 100, 2
                    )
                },
                "timestamp": datetime.utcnow().isoformat()
            }
...
async def execute_custom_query(
    ...
    limit: int = Query(100, ge=1, le=1000, description="Result limit for SELECT queries"),
    current_user: User = Depends(get_current_superuser),
    db: AsyncSession = Depends(get_db),
):
    """
