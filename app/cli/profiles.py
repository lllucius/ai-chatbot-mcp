...
profile_app = typer.Typer(
    help="🎛️ LLM parameter profile management commands",
...
def list_profiles(
    active_only: bool = typer.Option(True, "--active-only/--include-inactive", help="Show only active profiles"),
    search: Optional[str] = typer.Option(None, "--search", "-s", help="Search in name, title, and description"),
...
"""
                ...
                for profile in profiles:
                    ...
                    last_used = profile.last_used_at.strftime("%Y-%m-%d") if profile.last_used_at else "Never"
...
def show_profile(name: str = typer.Argument(..., help="Profile name")):
    ...
    async def _show_profile():
        try:
            ...
            if profile.stop:
                stop_str = json.dumps(profile.stop) if isinstance(profile.stop, list) else str(profile.stop)
                params_content.append(f"[bold]Stop Sequences:[/bold] {stop_str}")
            
            if profile.other_params:
                params_content.append(f"[bold]Other Parameters:[/bold] {json.dumps(profile.other_params, indent=2)}")
...
def add_profile(
    ...
    repeat_penalty: Optional[float] = typer.Option(None, "--repeat-penalty", help="Repetition penalty"),
...
"""
            ...
            if stats['recently_used']:
                ...
                for profile in stats['recently_used']:
                    last_used = profile['last_used_at'].strftime("%Y-%m-%d %H:%M") if profile['last_used_at'] else "Never"
                    table.add_row(
                        profile['name'],
                        profile['title'],
                        str(profile['usage_count']),
