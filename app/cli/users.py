...
def list(
    ...
    sort_by: str = typer.Option("created", "--sort", help="Sort by: id, username, email, created, updated"),
):
        ...
        async with AsyncSessionLocal() as db:
            try:
                ...
                for user in users:
                    ...
                    last_login = format_timestamp(user.last_login) if hasattr(user, 'last_login') and user.last_login else "Never"
    ...
    async def _delete_user():
        async with AsyncSessionLocal() as db:
            try:
                ...
                if not force:
                    if not Confirm.ask("Are you sure you want to delete this user and all associated data?"):
                ...
                if success:
                    success_message(f"User '{username}' and all associated data deleted successfully")
                else:
...
def reset_password(
    ...
    password: str = typer.Option(None, "--password", "-p", help="New password (will prompt if not provided)"),
):
    ...
    async def _user_stats():
        async with AsyncSessionLocal() as db:
            try:
                ...
                table.add_row("Active Users", str(active_users or 0), f"{(active_users or 0) / max(total_users or 1, 1) * 100:.1f}%")
                table.add_row("Inactive Users", str(inactive_users or 0), f"{(inactive_users or 0) / max(total_users or 1, 1) * 100:.1f}%")
                table.add_row("Superusers", str(superusers or 0), f"{(superusers or 0) / max(total_users or 1, 1) * 100:.1f}%")
                table.add_row("Recent (7 days)", str(recent_users or 0), f"{(recent_users or 0) / max(total_users or 1, 1) * 100:.1f}%")
