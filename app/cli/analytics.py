    ...
    async def _system_overview():
        async with AsyncSessionLocal() as db:
            try:
                ...
                user_activity = (active_convs / max(total_convs, 1)) * 100 if total_convs else 0
    ...
    async def _usage_stats():
        async with AsyncSessionLocal() as db:
            try:
                ...
                table.add_row("Active Users", str(active_users or 0), f"{(active_users or 0) / days:.1f}")
                table.add_row("New Users", str(new_users or 0), f"{(new_users or 0) / days:.1f}")
                table.add_row("New Documents", str(new_documents or 0), f"{(new_documents or 0) / days:.1f}")
                table.add_row("Processed Documents", str(processed_docs or 0), f"{(processed_docs or 0) / days:.1f}")
                table.add_row("New Conversations", str(new_conversations or 0), f"{(new_conversations or 0) / days:.1f}")
                table.add_row("New Messages", str(new_messages or 0), f"{(new_messages or 0) / days:.1f}")
                table.add_row("Tokens Used", str(tokens_used or 0), f"{(tokens_used or 0) / days:.0f}")
    ...
    async def _performance_metrics():
        async with AsyncSessionLocal() as db:
            try:
                ...
                table.add_row("Document Processing", "Success Rate", f"{success_rate:.1f}%", f"{status_icon} {'Excellent' if success_rate > 90 else 'Good' if success_rate > 70 else 'Poor'}")
                ...
                conv_efficiency = "🟢 High" if avg_messages_per_conv and avg_messages_per_conv > 5 else "🟡 Moderate"
                table.add_row("Conversations", "Total Conversations", str(total_convs or 0), "")
                table.add_row("", "Avg Messages/Conv", f"{avg_messages_per_conv or 0:.1f}", conv_efficiency)
                table.add_row("", "Total Messages", str(await db.scalar(select(func.count(Message.id))) or 0), "")
                
                # Token efficiency
                table.add_row("", "", "", "")  # Separator
                token_efficiency = "🟢 Efficient" if avg_tokens_per_message and avg_tokens_per_message < 500 else "🟡 Moderate"
                table.add_row("Token Usage", "Total Tokens", str(total_tokens or 0), "")
                table.add_row("", "Avg Tokens/Message", f"{avg_tokens_per_message or 0:.0f}", token_efficiency)
                ...
                if processing_docs and processing_docs > 10:
                    recommendations.append("Many documents in processing queue - check background workers")
...
def users(
    ...
    metric: str = typer.Option("messages", "--metric", "-m", help="Sort by: messages, documents, conversations"),
):
        ...
        async with AsyncSessionLocal() as db:
            try:
                if metric == "messages":
                    ...
                    error_message(f"Invalid metric: {metric}. Use messages, documents, or conversations")
                    return
        ...
        async with AsyncSessionLocal() as db:
            try:
                ...
                for i in range(intervals):
                    ...
                    period_label = period_start.strftime("%m-%d" if interval_days == 1 else "%m-%d to")
                    if interval_days > 1:
                ...
                if intervals >= 2:
                    ...
                    last_messages = await db.scalar(
                        select(func.count(Message.id))
                        .where(Message.created_at >= last_period_start)
                    )
                    
                    if first_messages and last_messages:
                        growth_rate = ((last_messages - first_messages) / first_messages) * 100
                        growth_status = "📈 Growing" if growth_rate > 0 else "📉 Declining" if growth_rate < -10 else "➡️ Stable"
                        
                        growth_panel = Panel(
                            f"Message Activity Growth: {growth_rate:+.1f}%\n{growth_status}",
                            title="📊 Growth Analysis",
                            border_style="green" if growth_rate > 0 else "red" if growth_rate < -10 else "yellow"
                        )
        ...
        async with AsyncSessionLocal() as db:
            try:
                ...
                report_data["system_overview"] = {
                    ...
                    "completed_documents": await db.scalar(select(func.count(Document.id)).where(Document.status == FileStatus.COMPLETED)) or 0,
