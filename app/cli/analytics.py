"Command-line interface for analytics management."
...
[bold blue]Active:[/bold blue] {(active_users or 0)}
[bold yellow]New (24h):[/bold yellow] {(new_users_24h or 0)}""",
                    title="👥 Users",
                    border_style="green",
                )
                doc_panel = Panel(
                    f"""[bold green]Total:[/bold green] {(total_docs or 0)}
[bold blue]Processed:[/bold blue] {(completed_docs or 0)}
[bold yellow]Storage:[/bold yellow] {format_size((total_storage or 0))}
[bold cyan]New (24h):[/bold cyan] {(docs_24h or 0)}""",
                    title="📄 Documents",
                    border_style="blue",
                )
                conv_panel = Panel(
                    f"""[bold green]Total:[/bold green] {(total_convs or 0)}
[bold blue]Active:[/bold blue] {(active_convs or 0)}
[bold yellow]Messages:[/bold yellow] {(total_messages or 0)}
[bold cyan]Msgs (24h):[/bold cyan] {(messages_24h or 0)}""",
                    title="💬 Conversations",
                    border_style="yellow",
                )
                processing_rate = (
    ...
    async def _performance_metrics():
        ...
        async with AsyncSessionLocal() as db:
            try:
                ...
                table.add_row(
                    "", "Currently Processing", str((processing_docs or 0)), ""
                )
                table.add_row(
                ...
                table.add_row(
                    "Token Usage", "Total Tokens", str((total_tokens or 0)), ""
                )
                table.add_row(
                    "",
                    "Avg Tokens/Message",
                ...
                storage_per_user = (
                    (
                        total_storage
                        / max((await db.scalar(select(func.count(User.id)))), 1)
                    )
                    if total_storage
                    else 0
                )
            ...
            try:
                if metric == "messages":
                    ...
                    for i, (username, email, msg_count, total_tokens) in enumerate(
                        ...
                        avg_tokens = (
                            ((total_tokens or 0) / max(msg_count, 1))
                            if msg_count
                            else 0
                        )
                        table.add_row(
                    ...
                    if first_messages and last_messages:
                        ...
                        growth_status = (
                            ...
                            if (growth_rate > 0)
                                ...
                                "📉 Declining" if (growth_rate < (-10)) else "➡️ Stable"
                            )
                        )
                        growth_panel = Panel(
        ...
        async with AsyncSessionLocal() as db:
            try:
                ...
                report_data["system_overview"] = {
                    ...
                    "total_storage_bytes": (
                        (await db.scalar(select(func.sum(Document.file_size)))) or 0
                    ),
                    "total_tokens": (
                        (await db.scalar(select(func.sum(Message.token_count)))) or 0
                    ),
                }
                if format == "json":
                    import json
