...
prompt_app = typer.Typer(
    help="📝 Prompt management commands",
...
def list_prompts(
    active_only: bool = typer.Option(True, "--active-only/--include-inactive", help="Show only active prompts"),
    category: Optional[str] = typer.Option(None, "--category", "-c", help="Filter by category"),
    search: Optional[str] = typer.Option(None, "--search", "-s", help="Search in name, title, content, and tags"),
...
"""
                ...
                for prompt in prompts:
                    ...
                    last_used = prompt.last_used_at.strftime("%Y-%m-%d") if prompt.last_used_at else "Never"
...
def add_prompt(
    ...
    description: Optional[str] = typer.Option(None, "--description", "-d", help="Prompt description"),
...
"""
            ...
            if stats['recently_used']:
                ...
                for prompt in stats['recently_used']:
                    last_used = prompt['last_used_at'].strftime("%Y-%m-%d %H:%M") if prompt['last_used_at'] else "Never"
                    table.add_row(
                        prompt['name'],
                        prompt['title'],
                        str(prompt['usage_count']),
