...
def upload(
    ...
    username: str = typer.Option(None, "--user", "-u", help="Username of document owner (defaults to system)"),
...
def list(
    ...
    status: str = typer.Option(None, "--status", "-s", help="Filter by status (pending, processing, completed, failed)"),
    ...
    sort_by: str = typer.Option("created", "--sort", help="Sort by: id, title, size, status, created"),
):
        ...
        async with AsyncSessionLocal() as db:
            try:
                ...
                if chunk_count and chunk_count > 0:
                    ...
                    for chunk in chunks:
                        preview = chunk.content[:57] + "..." if len(chunk.content) > 60 else chunk.content
                        chunk_table.add_row(
                            str(chunk.chunk_index),
                            preview,
                            str(chunk.token_count or 0)
                        )
    ...
    async def _delete_document():
        async with AsyncSessionLocal() as db:
            try:
                ...
                if chunk_count:
                    await db.execute(
                        DocumentChunk.__table__.delete()
                        .where(DocumentChunk.document_id == document_id)
                    )
                ...
                success_message(f"Document '{document.title}' and {chunk_count or 0} chunks deleted successfully")
    ...
    async def _search_documents():
        async with AsyncSessionLocal() as db:
            try:
                ...
                if not results:
                    warning_message(f"No documents found matching '{query}' with threshold {threshold}")
                    return
    ...
    async def _document_stats():
        async with AsyncSessionLocal() as db:
            try:
                ...
                table.add_row("Total Size", format_size(total_size or 0), f"Avg: {format_size(avg_size or 0)}")
                table.add_row("Total Chunks", str(total_chunks or 0), f"Avg per doc: {avg_chunks_per_doc:.1f}")
                table.add_row("Recent (7 days)", str(recent_docs or 0), f"{(recent_docs or 0) / max(total_docs or 1, 1) * 100:.1f}% of total")
                ...
                for status, count in status_counts.items():
                    ...
                    percentage = f"{count / max(total_docs or 1, 1) * 100:.1f}%" if total_docs else "0%"
