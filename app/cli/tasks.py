"Command-line interface for tasks management."
            ...
            if worker_result.returncode == 0:
                try:
                    ...
                    if worker_data:
                        ...
                        total_active = sum(
                            (len(tasks) for tasks in worker_data.values())
                        )
        ...
        try:
            ...
            try:
                ...
                for worker_name, stats in stats_data.items():
                    ...
                    memory_mb = (
                        (stats.get("rusage", {}).get("maxrss", 0) / 1024)
                        if stats.get("rusage", {}).get("maxrss")
                        else 0
                    )
                ...
                if registered_result.returncode == 0:
                    try:
                        registered_data = (
                            json.loads(registered_result.stdout)
                            if registered_result.stdout.strip()
                            else {}
                        )
                        if registered_data:
    ...
    def _purge_queue():
        ...
        try:
            if not force:
                ...
                if not Confirm.ask(
                    f"Are you sure you want to purge all tasks from queue '{queue}'?"
                ):
...
def monitor(
    ...
    def _monitor_tasks():
        ...
        try:
            ...
            console.print(
                f"[bold]Monitoring tasks for {duration} seconds (refresh every {refresh}s)[/bold]"
            )
            ...
            try:
                while datetime.now() < end_time:
                    console.print(("\n" * 2))
                    console.print(
                        f"[bold]Task Monitor - {datetime.now().strftime('%H:%M:%S')}[/bold]"
                    )
                    console.print(("=" * 60))
                    try:
                        import redis
    ...
    async def _task_stats():
        ...
        try:
            async with AsyncSessionLocal() as db:
                ...
                stats_table.add_row(
                    "Total Documents", str((total_docs or 0)), "100%", ""
                )
                stats_table.add_row(
                    "Pending",
                    str((pending_docs or 0)),
                    f"{(((pending_docs or 0) / max(total_docs, 1)) * 100):.1f}%",
                    "⏳ Waiting",
                )
                stats_table.add_row(
                    "Processing",
                    str((processing_docs or 0)),
                    f"{(((processing_docs or 0) / max(total_docs, 1)) * 100):.1f}%",
                    "⚙️ Working",
                )
                stats_table.add_row(
                    "Completed",
                    str((completed_docs or 0)),
                    f"{success_rate:.1f}%",
                    "✅ Success",
                )
                stats_table.add_row(
                    "Failed",
                    str((failed_docs or 0)),
                    f"{failure_rate:.1f}%",
                    "❌ Failed",
                )
                stats_table.add_row("", "", "", "")
