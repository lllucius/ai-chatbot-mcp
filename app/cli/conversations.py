...
def list(
    ...
    sort_by: str = typer.Option("updated", "--sort", help="Sort by: id, title, created, updated, messages"),
):
        ...
        async with AsyncSessionLocal() as db:
            try:
                ...
                for conv in conversations:
                    username_display = users.get(conv.user_id, "System") if conv.user_id else "System"
                ...
                if show_messages and message_count and message_count > 0:
                    ...
                    if messages:
                        ...
                        for msg in messages:
                            ...
                            content_preview = msg.content[:57] + "..." if len(msg.content) > 60 else msg.content
            ...
            try:
                ...
                if format == "json":
                    ...
                    with open(output_path, 'w', encoding='utf-8') as f:
                        ...
                        for msg in messages:
                            f.write(f"[{msg.created_at.strftime('%Y-%m-%d %H:%M:%S')}] {msg.role.upper()}:\n")
    ...
    async def _delete_conversation():
        async with AsyncSessionLocal() as db:
            try:
                ...
                success_message(f"Conversation '{conversation.title}' and {message_count or 0} messages deleted successfully")
...
@conversation_app.command()
def archive(
    older_than: int = typer.Option(90, "--older-than", help="Archive conversations older than N days"),
    ...
    async def _archive_conversations():
        async with AsyncSessionLocal() as db:
            try:
                ...
                if not conversations:
                    info_message(f"No conversations older than {older_than} days found for archiving")
                    return
                
                console.print(f"\n[bold yellow]Found {len(conversations)} conversations to archive:[/bold yellow]")
                for conv in conversations[:10]:  # Show first 10
                    console.print(f"  • {conv.title} (ID: {conv.id}, Updated: {conv.updated_at.strftime('%Y-%m-%d')})")
        ...
        async with AsyncSessionLocal() as db:
            try:
                ...
                recent_convs = await db.scalar(
                    select(func.count(Conversation.id)).where(Conversation.created_at >= last_7_days)
                )
                ...
                table.add_row("Active Conversations", str(active_convs or 0), f"{(active_convs or 0) / max(total_convs or 1, 1) * 100:.1f}%")
                table.add_row("Inactive Conversations", str(inactive_convs or 0), f"{(inactive_convs or 0) / max(total_convs or 1, 1) * 100:.1f}%")
                table.add_row("Total Messages", str(total_messages or 0), f"Avg per conv: {avg_messages_per_conv:.1f}")
                table.add_row("Users with Conversations", str(users_with_convs or 0), "")
                
                # Recent activity
                table.add_row("", "", "")  # Separator
                table.add_row("Recent Conversations (7d)", str(recent_convs or 0), f"{(recent_convs or 0) / max(total_convs or 1, 1) * 100:.1f}% of total")
                table.add_row("Recent Messages (7d)", str(recent_messages or 0), f"{(recent_messages or 0) / max(total_messages or 1, 1) * 100:.1f}% of total")
                
                # Message roles
                table.add_row("", "", "")  # Separator
                for role, count in role_stats.items():
                    percentage = f"{count / max(total_messages or 1, 1) * 100:.1f}%" if total_messages else "0%"
                    table.add_row(f"Messages ({role})", str(count), percentage)
                
                # Tokens
                table.add_row("", "", "")  # Separator
                table.add_row("Total Tokens", str(total_tokens or 0), f"Avg per msg: {avg_tokens_per_msg or 0:.1f}")
