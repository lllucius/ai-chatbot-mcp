...
STANDARD_LIBRARY_MODULES = {
    'asyncio', 'datetime', 'functools', 'json', 'logging', 'math', 'os', 
    'pathlib', 'sys', 'time', 'typing', 'uuid', 'warnings', 're'
}

THIRD_PARTY_MODULES = {
    'fastapi', 'uvicorn', 'sqlalchemy', 'asyncpg', 'pydantic', 'jose', 
    'passlib', 'openai', 'tiktoken', 'numpy', 'httpx'
}
                ...
                if current_group and group != current_group:
                    if (current_group == 'standard' and group == 'local') or \
                       (current_group == 'third_party' and group == 'standard'):
                        violations.append(f"Line {line_num}: Import order violation - {group} after {current_group}")
