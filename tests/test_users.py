...
class TestUserValidation:
    ...
    @pytest.mark.unit
    def test_register_user_invalid_username(self, client: TestClient, sample_user_data: dict):
    ...
    def test_register_user_valid_data(self, client: TestClient, test_factory):
        ...
        valid_users = [
            test_factory.create_user_data("user1", "user1@example.com", "ValidPass123!"),
            test_factory.create_user_data("test_user_2", "test2@domain.org", "AnotherPass456@"),
            test_factory.create_user_data("developer123", "dev@company.co.uk", "DevPassword789#"),
        ]
        
        for i, user_data in enumerate(valid_users):
            response = client.post("/api/v1/auth/register", json=user_data)
            assert response.status_code == status.HTTP_201_CREATED, f"Failed for user {i+1}: {user_data}"
