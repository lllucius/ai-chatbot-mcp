...
async def check_database_connection():
    "Check Database Connection operation."
    try:
        async with AsyncSessionLocal() as db:
...
async def check_openai_connection():
    "Check Openai Connection operation."
    try:
        ...
        if health_result.get("openai_available") and (
            ...
            if chat_available and embedding_available:
                ...
                logger.warning(
                    "⚠️  OpenAI chat model available, but embedding model failed"
                )
                return True
            else:
...
async def check_fastmcp_services():
    "Check Fastmcp Services operation."
    try:
        ...
        if healthy_servers == 0:
            ...
            return False
        logger.info(
            f"✅ FastMCP services: {healthy_servers}/{total_servers} servers healthy"
        )
        ...
        for required_server in required_servers:
            if (required_server not in server_status) or (
                server_status[required_server].get("status") != "healthy"
            ):
...
async def setup_directories():
    "Setup Directories operation."
    directories = [settings.upload_directory, "logs", "migrations/versions"]
...
async def run_startup():
    ...
    try:
        ...
        if not fastmcp_ok:
            logger.error(
                "❌ FastMCP services failed - this is REQUIRED for the application"
            )
        ...
        logger.info(
            f"🤖 OpenAI: {('✅ Ready' if openai_ok else '⚠️  Not configured (optional)')}"
        )
        logger.info(
            f"🔧 FastMCP: {('✅ Ready (REQUIRED)' if fastmcp_ok else '❌ FAILED (REQUIRED)')}"
        )
