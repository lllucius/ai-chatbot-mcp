...
class CodeQualityChecker:
    ...
    def check_all(self) -> Dict[str, List[str]]:
        ...
        for file_path in python_files:
            ...
            results['docstring_completeness'].extend(
    ...
    def check_logging_patterns(self, file_path: Path) -> List[str]:
        ...
        try:
            ...
            if "logger." in content and "StructuredLogger" not in content and "BaseService" not in content:
                violations.append(
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.ClassDef) and node.name.endswith('Service'):
                    if node.name != 'BaseService':
                        ...
                        inherits_base = any(
                            getattr(base, 'id', None) == 'BaseService' or 
                            getattr(base, 'attr', None) == 'BaseService'
    ...
    def generate_report(self, results: Dict[str, List[str]]) -> str:
        ...
        for category, violations in results.items():
            if violations:
                report.append(f"{category.replace('_', ' ').title()}: {len(violations)} violations")
