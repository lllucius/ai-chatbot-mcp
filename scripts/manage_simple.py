"Script for manage simple operations."
...
class ManagementCLI:
    "ManagementCLI class for specialized functionality."
        ...
        async with AsyncSessionLocal() as db:
            try:
                ...
                (users, total) = await user_service.list_users(
                    page=1, size=100, active_only=active_only
                )
                if not users:
                ...
                print(("-" * 80))
                print(
                    f"{'ID':<5} {'Username':<20} {'Email':<30} {'Active':<8} {'Super':<8} {'Created'}"
                )
                print(("-" * 80))
                for user in users:
                    ...
                    print(
                        f"{user.id:<5} {user.username:<20} {user.email:<30} {active:<8} {super_user:<8} {created}"
                    )
            except Exception as e:
    ...
    def show_examples(self):
        "Show Examples operation."
        print("🔧 MANAGEMENT CLI EXAMPLES")
        print(("=" * 50))
        print("Create a regular user:")
        print(
            "  python scripts/manage_simple.py create-user john john@example.com SecurePass123"
        )
        print()
...
async def main():
    ...
    create_super_parser = subparsers.add_parser(
    ...
    list_users_parser.add_argument(
        "--active-only", action="store_true", help="Show only active users"
    )
    reset_pass_parser = subparsers.add_parser(
    ...
    deactivate_parser = subparsers.add_parser(
        "deactivate-user", help="Deactivate a user"
    )
