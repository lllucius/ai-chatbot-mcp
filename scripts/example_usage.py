...
class APIClient:
    ...
    async def register(self, username: str, email: str, password: str, full_name: str = None) -> Dict[str, Any]:
        """Register a new user."""
        data = {
            "username": username,
            "email": email,
    ...
    async def search_documents(self, query: str, algorithm: str = "hybrid") -> Dict[str, Any]:
        """Search documents."""
        data = {
            "query": query,
            "algorithm": algorithm,
    ...
    async def chat(self, message: str, conversation_id: int = None, use_rag: bool = True) -> Dict[str, Any]:
        """Send a chat message."""
        data = {
...
async def demonstrate_workflow():
    ...
    try:
        ...
        upload_result = await client.upload_document(str(sample_file), "AI Introduction")
        ...
        # Chat with AI
        print("\n9. 💬 Chatting with AI...")
        chat_result = await client.chat("What is the difference between AI and machine learning?")
    ...
    try:
        ...
        if reg_result.get("username"):
            ...
            if "access_token" in login_result:
                ...
                # Get profile
                profile = await client.get_profile()
                print(f"   👤 Profile: {profile.get('full_name')} ({profile.get('email')})")
            else:
                print("   ❌ Login failed")
        else:
