...
async def create(
    ...
    password: Optional[str] = Option(None, "--password", "-p", help="Password (will prompt if not provided)"),
    full_name: Optional[str] = Option(None, "--full-name", help="Full name of the user"),
        ...
        if superuser and not user.is_superuser:
            try:
                ...
                if getattr(promote_result, "success", False):
                    ...
                    error_message(f"Failed to promote user to superuser: {getattr(promote_result, 'message', '')}")
...
async def list(
    ...
    superuser_only: bool = Option(False, "--superuser-only", help="Show only superusers"),
    search: Optional[str] = Option(None, "--search", help="Search users by username or email"),
):
