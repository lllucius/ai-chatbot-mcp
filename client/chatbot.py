...
class AIChatbotTerminal:
    ...
    def load_conversations(self):
        ...
        for i, conv in enumerate(convos.items, 1):
            ...
            last_msg = conv.last_message_at.strftime('%Y-%m-%d %H:%M') if conv.last_message_at else "No messages"
            print(
    ...
    def configure_settings(self):
        ...
        # Toggle Tools
        tools_input = input_prompt(f"Enable Tools [{self.config.default_use_tools}]: ").strip().lower()
        ...
        # Max history
        history_input = input_prompt(f"Max History Display [{self.config.max_history_display}]: ").strip()
    ...
    def handle_tools_command(self, cmd: str):
        ...
        try:
            if action == "list":
                ...
                try:
                    if action == "enable":
                        result = self.sdk.tools.enable_tool(tool_name)
                        print(f"Enabled tool: {tool_name}")
                    else:
                        result = self.sdk.tools.disable_tool(tool_name)
    ...
    def handle_docs_command(self, cmd: str):
        ...
        try:
            if action == "list":
                ...
                query = parts[2]
                try:
                    results = self.sdk.search.search(DocumentSearchRequest(q=query, page=1, per_page=10))
                    if results.get('results'):
                        print(f"Found {len(results['results'])} results for '{query}':")
                        for result in results['results']:
                            print(f"  {result.get('title', 'Unknown')} (Score: {result.get('score', 0):.2f})")
